const CONSENT_KEY = "rip.ads.consent.v1";
...
function createAdFrame(slotName) {
  ...
  if (!isNative && !format) return null;

  const frame = document.createElement("iframe");
  frame.className = `ad-frame ad-frame-${requested}`;
  frame.title = "Contenu publicitaire";
  frame.loading = "lazy";
  frame.referrerPolicy = "origin";
  frame.setAttribute("sandbox", "allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox");
  frame.width = String(isNative ? 720 : format.width);
  frame.height = String(isNative ? 280 : format.height);
  frame.srcdoc = isNative ? nativeFrameDocument() : frameDocument(format);
  return frame;
}
